...
### An example that shows what this compiler is capable of
...
Run the compiler with the file name as the only commandline argument. It would then print out the assembly code. Save the assembly code as `LinkedList.s`, and use `gcc LinkedList.s` on a 32-bit Linux.

You can see that this program uses quite a lot of features: pointers, structs(especially when a member refers to the struct type itself), loops, functions... This compiler supports the whole ISO C. However, it doesn't contain a preprocessor, so you can't directly use NULL (which is `#define`ed as `(void *)0`).
...
### 3. Semantic Analysis - mostly finished, but left with some holes.
